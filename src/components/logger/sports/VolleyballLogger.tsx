...
export const VolleyballLogger: React.FC<VolleyballLoggerProps> = ({
  ...
  const volleyballEvents: CampusEventType[] = [
  ...
  const handleSubmitEvent = () => {
    ...
    if (!selectedPlayerId && selectedEventType !== 'error') {
      toast({
        title: 'Error',
    ...
    toast({
      title: 'Event Logged',
      description: `${selectedEventType} recorded for ${selectedTeam?.name || 'Unknown Team'}`
    });
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="text-center">
              <div className="text-2xl font-bold">{teams[0]?.name || 'Team 1'}</div>
            ...
            <div className="text-center">
              <div className="text-lg font-semibold">Set {setNumber}</div>
              <div className="my-2">
                <Button 
                  onClick={() => setSetNumber(prev => Math.max(1, prev - 1))} 
                  disabled={disabled}
                  variant="outline"
                  size="sm"
                >
                ...
                <Button 
                  onClick={() => setSetNumber(prev => prev + 1)} 
                  disabled={disabled}
                  variant="outline"
                  size="sm"
                  className="ml-2"
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{teams[1]?.name || 'Team 2'}</div>

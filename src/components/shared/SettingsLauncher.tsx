...
export default function SettingsLauncher() {
  ...
  return (
    <>
      ...
      <div className={`fixed ${hasPageHeaderControls ? 'top-16' : 'top-4'} right-30 z-40 pointer-events-none`}>
        <button
          aria-label="Open settings"
          onClick={() => setOpen(true)}
          className="pointer-events-auto w-10 h-10 rounded-full flex items-center justify-center text-gray-700 hover:bg-black/5 dark:text-white dark:hover:bg-white/10 transition-colors border border-black/10 dark:border-white/10"
        >

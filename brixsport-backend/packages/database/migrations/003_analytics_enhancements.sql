...
-- Immutable helper functions for index expressions
CREATE OR REPLACE FUNCTION immutable_extract_utc_hour(ts TIMESTAMP WITH TIME ZONE)
RETURNS INTEGER
LANGUAGE sql
IMMUTABLE
AS $$
    SELECT EXTRACT(HOUR FROM ts AT TIME ZONE 'UTC')::INTEGER;
$$;

CREATE OR REPLACE FUNCTION immutable_created_utc_date(ts TIMESTAMP WITH TIME ZONE)
RETURNS DATE
LANGUAGE sql
IMMUTABLE
AS $$
...
CREATE INDEX IF NOT EXISTS idx_user_activity_logs_created_at_hour ON user_activity_logs(immutable_extract_utc_hour(created_at));
CREATE INDEX IF NOT EXISTS idx_user_activity_logs_created_at_date ON user_activity_logs(immutable_created_utc_date(created_at));
CREATE INDEX IF NOT EXISTS idx_user_activity_logs_user_action_date ON user_activity_logs(user_id, action, immutable_created_utc_date(created_at));

...
export default function LoginFormClient({ initialUserType = 'admin' }: LoginFormClientProps) {
  const { login, demoLogin } = useAuth();
  const router = useRouter();
  const searchParams = useSearchParams();
  const [userType, setUserType] = useState<'admin' | 'logger'>(initialUserType);

  // Check if we should default to a specific user type
  React.useEffect(() => {
    const type = searchParams.get('type');
    if (type === 'admin' || type === 'logger') {
      setUserType(type);
    }
  }, [searchParams]);
